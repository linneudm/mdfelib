...
class MdfeUrls(Enum):
    ...
    """
    RECEPCAO = "svrs.rs.gov.br/ws/MDFeRecepcao/MDFeRecepcao.asmx" #0
    RETRECEPCAO = "svrs.rs.gov.br/ws/MDFeRetRecepcao/MDFeRetRecepcao.asmx" #1
    RECEPCAOEVENTO = "svrs.rs.gov.br/ws/MDFeRecepcaoEvento/MDFeRecepcaoEvento.asmx" #2
...
class MountMDFeType(GeneratedsSuper):
    ...
    def hasContent_(self):
        if (
            ...
            return False
    def export(self, outfile, level, namespace_='', name_='enviMDFe', namespacedef_='', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('enviMDFe')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        ...
        outfile.write('<%s%s%s' % (namespace_, name_, namespacedef_ and ' ' + namespacedef_ or '', ))
        already_processed = set()
        self.exportAttributes(outfile, level, already_processed, namespace_, name_='enviMDFe')
        if self.hasContent_():
            outfile.write('>%s' % (eol_, ))
            self.exportChildren(outfile, level + 1, namespace_='', name_='enviMDFe', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write('</%s%s>%s' % (namespace_, name_, eol_))
        else:
            outfile.write('/>%s' % (eol_, ))
    def exportAttributes(self, outfile, level, already_processed, namespace_='', name_='enviMDFe'):
        if self.versao is not None and 'versao' not in already_processed:
            already_processed.add('versao')
            outfile.write(' versao=%s' % (quote_attrib(self.versao), ))
        if self.Id is not None and 'Id' not in already_processed:
    ...
    def exportChildren(self, outfile, level, namespace_='', name_='enviMDFe', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
